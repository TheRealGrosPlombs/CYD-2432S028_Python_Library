...
class Touch(object):
    ...
    def __init__(self, spi, cs, int_pin=None, int_handler=None,
        ...
        Args:
            ...
            lcd: the lcd object used to draw (used to match ts coordinates to lcd coordinates)
        """
        self.lcd = lcd
        self.spi = spi
        self.cs = cs
    ...
    def normalize(self, x, y):
        ...
        if self.lcd is None:
            return x, y
        # convert x,y touch coordinates to match LCD rotation
        # display_lcd_rot == 270
        cx = y
        cy = x
        #print(f"rot:{self.lcd.lcd_rot}, w:{self.lcd.width}, h:{self.lcd.height}")
        if self.lcd.rotation == 0:
            cx = x
            cy = self.lcd.height - y
        if self.lcd.rotation == 90:
            cx = self.lcd.width - y
            cy = self.lcd.height - x
        if self.lcd.rotation == 180:
            cx = self.lcd.width - x
            cy = y
        return cx, cy
